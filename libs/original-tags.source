 ...
 * Tags library
 * ============
 * This libray uses dashes to separate words in identifiers. Because the dash is not allowed in typescript identifiers, we use the half width Hnagul
...
const log = (...args: Tags.printable[]) =>
  console.log(
    args.reduce(
      (text: string, value: Tags.printable) => text.concat(value.toString()),
      ""
    )
  );
const test = (
...
const indentｰtext = `
        ...
        And we stay there.
            ...
            literal ${{ c: 3, d: { e: 4, f: [5, 6] } } as Tags.printable}
...
const numberingｰoptions: Tags.numberingｰoptions = {
  padｰwith: " ",
  numberｰfrom: -25,
  // prefix: "«",
  // suffix: "» ",
...
test(
  Tags.numbering(numberingｰoptions)(Tags.wrap(40)(Tags.outdent)),
  indentｰtext
);

test(Tags.numberｰlines(Tags.fraktur(Tags.wrap(40)(Tags.outdent))), indentｰtext);
...
test(
  Tags.numbering({})(Tags.paragraph(Tags.outdent)),
  Tags.pretty`
         This is some text with π = ${Math.PI}.
         This line has the same indentation as the previous one.
             This line has deeper indentation.
...
test(
  Tags.identity,
  Tags.pretty`Trying json on ${numberingｰoptions} and on ${[
    1, 2, 3,
  ]} plus on ${ab} and ${{ c: 3, d: { e: 4, f: [5, 6] } } as Tags.printable}`
);
